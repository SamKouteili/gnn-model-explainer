...
def json_to_networkx_semantic(data: Dict[str, Any], label: int, converter: SemanticAttributionGraphConverter) -> nx.Graph:
    ...
    for i, node_data in enumerate(valid_nodes):
        ...
        if np.isinf(feat).any():
            logger.warning(f"Found Inf in node features, replacing with finite values")
    ...
    for edge_data in edges_data:
        ...
        try:
            ...
            logger.warning(f"Non-numeric weight {weight} for edge {source_id}->{target_id}, setting to 1.0")
            weight = 1.0
            invalid_weight_edges += 1
        
        # Check for NaN/Inf weights
        if not (np.isfinite(weight) and not np.isnan(weight)):
            logger.warning(f"Invalid weight {weight} for edge {source_id}->{target_id}, setting to 1.0")
            weight = 1.0
            invalid_weight_edges += 1
    ...
    logger.info(f"Edge processing: {total_original_edges} original → {valid_edges} valid ({skipped_edges} skipped, {invalid_weight_edges} had invalid weights)")
    
    # Normalize edge weights for training stability
    if edge_weights:
        normalized_weights = converter._normalize_edge_weights_for_training(np.array(edge_weights))
...
def load_attribution_graphs_semantic(
    ...
    if benign_dir.exists():
        ...
        logger.info(f"Loading {len(benign_files)} benign graphs with semantic processing")
        for file_path in benign_files:
            try:
                with open(file_path, 'r') as f:
                    data = json.load(f)
                
                graph = json_to_networkx_semantic(data, label=0, converter=converter)
                graphs.append(graph)
                labels.append(0)
                logger.info(f"Successfully loaded {file_path.name}: {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges")
    ...
    if injected_dir.exists():
        ...
        logger.info(f"Loading {len(injected_files)} injected graphs with semantic processing")
        for file_path in injected_files:
            try:
                with open(file_path, 'r') as f:
                    data = json.load(f)
                
                graph = json_to_networkx_semantic(data, label=1, converter=converter)
                graphs.append(graph)
                labels.append(1)
                logger.info(f"Successfully loaded {file_path.name}: {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges")
            except Exception as e:
