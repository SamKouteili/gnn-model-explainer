...
def train_attribution_graphs(args):
    ...
    train_dataset, val_dataset, test_dataset, _, input_dim, assign_input_dim = prepare_data(
        all_graphs,  # All graphs for max_num_nodes calculation
        args,
    ...
    train_dataset = torch.utils.data.DataLoader(
        GraphSampler(train_graphs, normalize=False, max_num_nodes=max_num_nodes, features=args.feature_type),
    ...
    val_dataset = torch.utils.data.DataLoader(
        GraphSampler(val_graphs, normalize=False, max_num_nodes=max_num_nodes, features=args.feature_type),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
    )
    ...
    test_dataset = torch.utils.data.DataLoader(
        GraphSampler(test_graphs, normalize=False, max_num_nodes=max_num_nodes, features=args.feature_type),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
    )
...
def main():
    parser = argparse.ArgumentParser(description="Train GNN on attribution graphs")
    
    # Data arguments
    parser.add_argument("--datadir", required=True, help="Directory containing benign/ and injected/ subdirs")
    parser.add_argument("--cache-dir", help="Cache directory for processed graphs")
    parser.add_argument("--max-files", type=int, help="Maximum files per category (for testing)")
    
    # Model arguments
    parser.add_argument("--method", default="base", help="Method: base, soft-assign")
    parser.add_argument("--hidden-dim", type=int, default=64, help="Hidden dimension")
    parser.add_argument("--output-dim", type=int, default=64, help="Output dimension")
    parser.add_argument("--num-classes", type=int, default=2, help="Number of classes")
    parser.add_argument("--num-gc-layers", type=int, default=3, help="Number of GC layers")
    ...
    # Data processing
    parser.add_argument("--max-nodes", type=int, default=1000, help="Maximum number of nodes")
    parser.add_argument("--feature-type", default="default", help="Feature type: default, id, deg")
    parser.add_argument("--test-ratio", type=float, default=0.2, help="Test set ratio")
    parser.add_argument("--train-ratio", type=float, default=0.8, help="Train set ratio")
    ...
    parser.add_argument("--assign-ratio", type=float, default=0.1, help="Assignment ratio")
    parser.add_argument("--num-pool", type=int, default=1, help="Number of pooling layers")
    parser.add_argument("--linkpred", action="store_true", help="Use link prediction")
